...
//lab07-2
void StackType::Copy(StackType& otherStack) {
	NodeType *hostLoc;
	NodeType *predLoc = NULL;
	NodeType *otherLoc;

	if (otherStack.topPtr == NULL)
		topPtr = NULL;
	else {
		hostLoc = new NodeType;
		topPtr = hostLoc;
		otherLoc = otherStack.topPtr;
		while (otherLoc != NULL) {
			hostLoc->info = otherLoc->info;
			hostLoc->next = new NodeType;
			predLoc = hostLoc;
			hostLoc = hostLoc->next;
			otherLoc = otherLoc->next;
		}
		predLoc->next = NULL;
		delete hostLoc;
	}
